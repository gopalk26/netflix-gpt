...
import {checkValadition} from '../Utils/validation';
...
const Login = () => {
    ...
    const password =useRef(null);
     ...
     const handleButtonClick = ()=>{
      const message = checkValadition(email.current.value,password.current.value)
    ...
    <div>
      ...
      <form onSubmit={(e)=>e.preventDefault()} className="absolute w-3/12 p-12 my-36 mx-auto right-0 left-0 bg-black bg-opacity-75 rounded-lg">
        ...
        <input
          ...
          className="p-3 my-4 w-full font-sans rounded-lg border-2 border-rose-950 bg-transparent text-white focus:outline-none focus:border-gray-700"
        />
        <button className="p-3 my-4 bg-rose-600 w-full rounded-lg text-white hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-rose-600 "
           onClick={handleButtonClick}>
       {isSignIn ?"Sign In":"Sign Up"} 
        </button>
        <p className="text-red-600">{message}</p>
        <p className="py-4 text-white font-normal cursor-pointer" onClick={toggleSignForm}>{isSignIn?"New to Netflix ? Sign Up Now":"Already Registered Login"}</p>
      </form>
    </div>
  );
};
